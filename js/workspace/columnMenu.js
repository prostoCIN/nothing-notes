    ...
    window.App.columnMenu = {
        ...
        createMenu(options) {
            ...
            const renderDropdownContent = () => {
                ...
                // 1.5. Каунтер кількості нотаток у колонці
                const allColNotes = noteManager ? noteManager.getNotesForColumn(parentNoteId) : [];
                const totalNoteCount = allColNotes.length;

                const countItem = document.createElement('div');
                countItem.className = 'column-menu-item column-menu-item-count';
                countItem.title = t('column.notesCount', { count: totalNoteCount });
                const notesLabel = isRoot ? (t('sidebar.notes') || 'Нотатки') : (t('sticker.subnotesTitle') || 'Піднотатки');

                let badgeDisplay = totalNoteCount;
                if (currentColumnFilter && currentColumnFilter.size > 0 && noteManager) {
                    const filteredCount = allColNotes.filter(n => {
                        const noteTags = noteManager.getNoteTags(n);
                        return [...currentColumnFilter].every(t => noteTags.includes(t));
                    }).length;
                    badgeDisplay = `${filteredCount} / ${totalNoteCount}`;
                }

                countItem.innerHTML = `
                    <div class="column-menu-count-left">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="column-menu-count-icon">
                    ...
                    <span class="column-count-badge column-menu-count-badge">${badgeDisplay}</span>
                `;
